...
export const getRooms = async (req: Request, res: Response) => {
    try {
        ...
        for (const room of rooms) {
            ...
            (room as any).image = imgs.length > 0 ? imgs[0] : '/images/room-placeholder.jpg';
        }
        res.json(rooms);
    } catch (error: any) {
...
export const getRoomBySlug = async (req: Request, res: Response) => {
    try {
        ...
        if (room) {
            ...
            (room as any).images = roomImages.map(img => img.image_url);
            (room as any).image = roomImages.length > 0 && roomImages[0] ? roomImages[0].image_url : '/images/room-placeholder.jpg';
            res.json(room);
        } else {
